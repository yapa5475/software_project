	...
	*/
	...
	if (!empty($value) and !empty($value1) and !empty($value2) and !empty($value3) and !empty($value4) and !empty($value5) and !empty($value6)){
		...
		function checkPhoneNo($value6){
			...
			if(!$numDigits == 10) {
				echo "Invalid number of digits in phone number";
				break;
			}
			else{
				"valid phone number";
		...
		function checkPhoneNoAvail($value6){
			$query6 = mysql_query("SELECT Phoneno FROM Users WHERE Phoneno ='".$value6."'");		
			if (mysql_num_rows($query3) !=0){
		...
		//works. plz dont touch i spent too long on this ty v much (i'm talking to you yash)
		//works --- checks to make sure the passwords entered are equals
